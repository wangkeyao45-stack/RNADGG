...
sequences = df['full_sequence'].tolist()       # 修改列名
on_scores = df['ON'].values.astype(np.float32) # 修改列名 ON
off_scores = df['OFF'].values.astype(np.float32)# 修改列名 OFF
processed_data = []
    ...
    for t in iterator:
        ...
        if guidance_scale > 1e-6:
            with torch.enable_grad():
                ...
                # 计算梯度 (Gradient Ascent on Reward)
...
# --- 生成数据 ---
N_EVAL_SAMPLES = 200  # 评估样本数
BATCH_SIZE = 32
GUIDANCE_STRENGTH = 0.5  # 建议稍微加大一点以便观察效果
...
# ------------------------------------------------------------------------------
# 6.1 多目标分布散点图 (Multi-Objective Scatter Plot) [New!]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 6.2 综合得分分布 (Violin Plot)
# ------------------------------------------------------------------------------
print("Plotting Composite Reward Distribution...")
df_viol = pd.DataFrame({
    'Group': ['Real'] * len(comp_real) + ['Random'] * len(comp_rand) + ['Baseline'] * len(comp_base) + [
        'RL-Guided'] * len(comp_rl),
    'Composite Score': np.concatenate([comp_real, comp_rand, comp_base, comp_rl])
})
plt.figure(figsize=(10, 6))
sns.violinplot(x='Group', y='Composite Score', data=df_viol, palette="muted")
...
# ------------------------------------------------------------------------------
# 6.3 梯度动态可视化 (Gradient Dynamics) [New!]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 6.4 Top-5 序列分析与 Logo (Top-5 Analysis) [New!]
# ------------------------------------------------------------------------------
