...
output_dir = "output_plots_diffusion_utr_0.5_top5_2222"
    ...
    for t in tqdm(reversed(range(diffusion_model.T)), desc="Sampling (Max)", total=diffusion_model.T, leave=False):
        ...
        if current_scale > 1e-6:
            with torch.enable_grad():
                ...
                # 记录梯度信息 (为了画图)
                if track_gradients:
                    # 记录的是原始梯度范数，不受 scale 影响，以便观察预测器的置信度
...
def guided_sampling_target(diffusion_model, oracle_model, batch_size, target_value, guidance_scale=0.1, decay=True):
    ...
    for t in tqdm(reversed(range(diffusion_model.T)), desc=f"Sampling Target={target_value}", total=diffusion_model.T,
        ...
        if current_scale > 1e-6:
            ...
            with torch.enable_grad():
                x_t.requires_grad_()
                preds = oracle_model(x_t)  # 预测当前分数
                # 目标：最小化 (预测值 - 目标值)^2 -> Gradient Descent on Loss
                # 注意：扩散模型代码通常是 predicted_noise - scale * grad (假设grad是向上的)
                # 如果我们要最小化 Loss，我们需要梯度的反方向。
                # 这里的写法：loss = -MSE，然后求梯度，得到的是指向 MSE 变小方向的梯度（Gradient Ascent on Negative Loss）
                loss = -((preds - target_tensor) ** 2).sum()
                grad = torch.autograd.grad(loss, x_t)[0]
                grad = grad.clamp(-1, 1)  # 截断
...
# 设定3个目标值：低分、中分、高分
target_values = [5.0, 6.0, 7.0]
# 注意：目标值的范围应在训练集分数的合理范围内，太极端的数值模型可能生成不出来
# 您可以根据您的 rl_scores 数据分布调整这些值 (例如 mean, mean+std, max)
...
for target in target_values:
    ...
    # 使用新增的定值引导模式
    # 注意：定值生成通常需要比单纯最大化更大的 guidance_scale，这里设为 1.0 或 2.0 尝试
...
def plot_tsne_distribution():
    ...
    # 2. 整合数据
    # 将四个组的数据拼接起来
    # 确保维度一致，Part 7中 FID计算用的 N_EVAL_SAMPLES (200)，如果觉得点太少，可以由前面重新生成更多数据
    # 这里直接使用现有变量以保证代码流畅性
