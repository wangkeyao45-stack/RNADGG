...
# ==============================================================================
# ==============================================================================
# 第7部分 - 第4小节: 奖励分布对比 (精准配色修改版)
# ==============================================================================
print("\n--- Plotting Reward Distribution with Specified Colors ---")
...
df_viol = pd.DataFrame({
    'Method': ['Real'] * N_EVAL + ['Random'] * N_EVAL + ['Baseline'] * N_EVAL +
              ['RL-Guided'] * N_EVAL + ['Traditional GA'] * N_EVAL,
    'Score': np.concatenate([s_real, s_rand, s_base, s_rl, s_ga])
})

plt.figure(figsize=(12, 6))

# 修改点：将 palette 设置为我们定义的字典 custom_colors
sns.violinplot(
    x='Method',
    y='Score',
    data=df_viol,
...
def plot_tsne_distribution(methods_data, real_oh):
    ...
    plt.figure(figsize=(10, 8))
    # 在 plot_tsne_distribution 函数体内找到 sns.scatterplot
    sns.scatterplot(
        data=df_tsne,
        x='tsne_1',
        y='tsne_2',
        hue='Method',
        palette=custom_colors,  # 使用同样的颜色字典
        alpha=0.6,
        s=40
    )
    plt.title("Sequence Space Distribution (t-SNE)")
